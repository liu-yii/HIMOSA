...
from cem.SaliencyModel.utils import cv2_to_pil, pil_to_cv2
from cem.demo_ResultTable import CEM_Table
from cem.utils import generate_CE_map,plot_heatmap_PSNR,read_config
...
def main(config):
    ...
    for Model_name in Modellist:
        for Image_name in Imagelist:
            ...
            os.makedirs(f'./cem/{task}-CEM/{prefix}-R{ROI_size}M{mask_block_size}/{Model_name}/{Image_name}/', exist_ok=True)

            # Load PSNR data from .mat files
            O = \
            sio.loadmat(f'./cem/{task}-CEM/{prefix}-R{ROI_size}M{mask_block_size}/{Model_name}/{Image_name}-PSNR-Origin.mat')[
                'PSNR']
            Fine_perturb = sio.loadmat(
                f'./cem/{task}-CEM/{prefix}-R{ROI_size}M{mask_block_size}/{Model_name}/{Image_name}-PSNR_all_perturb-F{Fine_num}.mat')[
                'PSNR']
            Coarse_perturb = sio.loadmat(
                f'./cem/{task}-CEM/{prefix}-R{ROI_size}M{mask_block_size}/{Model_name}/{Image_name}-PSNR_all_perturb-C{Coarse_num}.mat')[
                'PSNR']
            ...
            pa = f'./cem/{task}-CEM/{prefix}-R{ROI_size}M{mask_block_size}/{Model_name}/{Image_name}/CEM_PSNR.png'
            position_pil2.save(pa)

    html_saving_path = f'./cem/{task}_{prefix}_Result.html'
